...
def test_compute_axis_ticks_returns_12_by_default():
    ...
    diffs = [ticks[i+1]["position_pct"] - ticks[i]["position_pct"] for i in range(len(ticks)-1)]
    for d in diffs:
...
def test_compute_axis_ticks_labels_are_taipei_hh_mm():
    ...
    import re
    for t in ticks:
        assert re.match(r"^\d{2}-\d{2} \d{2}:\d{2}$", t["label"]), f"bad label format: {t['label']}"
...
def test_compute_axis_ticks_custom_num():
    ...
    ticks = compute_axis_ticks("2026-08-04T00:00:00Z", "2026-08-04T06:00:00Z", num_ticks=6)
    assert len(ticks) == 6
...
def test_compute_axis_ticks_cross_midnight_shows_day_change():
    ...
    """
    ticks = compute_axis_ticks("2026-08-03T12:00:00Z", "2026-08-04T08:00:00Z", num_ticks=7)
