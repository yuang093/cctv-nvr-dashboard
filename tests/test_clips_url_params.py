...
def test_clips_html_reads_urlsearchparams():
    ...
    assert "new URLSearchParams(location.search)" in content, \
...
def test_clips_html_auto_selects_nvr():
    ...
    content = CLIPS_HTML.read_text(encoding="utf-8")
    assert "$nvr.value = pendingNvr" in content, \
        "應自動設定 $nvr.value"
    assert '$nvr.dispatchEvent(new Event("change"))' in content, \
...
def test_clips_html_auto_checks_cam():
    ...
    content = CLIPS_HTML.read_text(encoding="utf-8")
    assert "targetCb.checked = true" in content, \
        "應自動勾選對應 cam checkbox"


def test_clips_html_auto_plays_when_all_three_params_present():
    """三者齊全時應自動觸發同步撥放。"""
    content = CLIPS_HTML.read_text(encoding="utf-8")
    assert "autoPlayPending" in content, \
        "應有 autoPlayPending 旗標"
    assert "$syncPlayBtn.click()" in content, \
        "應自動 click 同步撥放按鈕"
...
def test_clips_html_fills_datetime_local_from_param():
    ...
    content = CLIPS_HTML.read_text(encoding="utf-8")
    assert "$t.value = pendingT" in content, \
        "應把 t 填入 datetime-local"
...
def test_clips_html_uses_css_escape_for_selector():
    ...
    content = CLIPS_HTML.read_text(encoding="utf-8")
    assert "cssEscape" in content, \
        "selector 應用 cssEscape 避免 injection"