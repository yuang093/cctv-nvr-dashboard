...
def test_get_timeline_uses_correct_endpoint(
    sample_nvr, mock_session, make_login_response
):
    ...
    s.login()
    s.get_timeline(
        "cam-1", from_iso="2026-07-28T00:00:00Z", to_iso="2026-07-29T00:00:00Z"
    )
...
def test_get_timeline_returns_parsed_data(
    sample_nvr, mock_session, make_login_response
):
...
def test_get_timeline_uses_server_id_from_server_ids_endpoint(
    sample_nvr, mock_session, make_login_response
):
