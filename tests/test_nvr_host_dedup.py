...
def test_bulk_upsert_dedup_same_host_different_nvr_id():
    ...
    result = bulk_upsert_nvrs(
        db_path,
        [
            {
                "nvr_id": "WIN-OPA34I3TCL5",
                "name": "Windows主機",
                "host": "192.168.133.141",
                "port": 8443,
                "username": "u",
    ...
    rows = conn.execute("SELECT id, nvr_id, host FROM nvr_servers").fetchall()
    conn.close()
...
def test_bulk_upsert_existing_nvr_id_updates_normally():
    ...
    conn = sqlite3.connect(db_path)
    row = conn.execute("SELECT name FROM nvr_servers WHERE nvr_id='NVR-A'").fetchone()
    conn.close()
    assert row[0] == "新名"
...
def test_bulk_upsert_dedup_migrates_all_related_data():
    ...
    conn = sqlite3.connect(db_path)
    nvrs = conn.execute("SELECT id, nvr_id FROM nvr_servers").fetchall()
    conn.close()
    ...
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM image_health_checks WHERE nvr_server_id=?",
            (new_internal_id,),
        ).fetchone()[0]
        == 1
    ), "image_health 應被 migrate"
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM recording_status WHERE nvr_id=?", (new_internal_id,)
        ).fetchone()[0]
        == 1
    ), "recording_status 應被 migrate"
    conn.close()
...
def test_bulk_upsert_dedup_only_affects_colliding_host():
    ...
    # 2 筆 NVR 都在
    assert conn.execute("SELECT COUNT(*) FROM nvr_servers").fetchone()[0] == 2
    conn.close()
