...
def test_run_web_ps1_default_is_localhost():
    ...
    assert m.group(1) == "127.0.0.1", (
        f"預設應為 127.0.0.1，got {m.group(1)!r}"
    )
...
def test_run_web_bat_default_is_localhost():
    ...
    assert 'set "NVR_WEB_HOST=127.0.0.1"' in bat, (
        "run_web.bat 預設 host 應為 127.0.0.1"
    )
...
def test_csv_template_no_plaintext_password():
    ...
    m = re.search(
        r'example_csv\s*=\s*\((.*?)\)', app_py, re.DOTALL
    )
    ...
    # 應有 <CHANGE_ME> 佔位符
    assert "<CHANGE_ME>" in csv_block, (
        "CSV template 應使用 <CHANGE_ME> 佔位符"
    )
