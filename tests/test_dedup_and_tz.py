...
def test_insert_events_dedup_same_topic_open(db_path):
    ...
    w.finish_scan_run(
        run1,
        finished_at="2026-07-03T05:02:00Z",
        status="success",
        stats={
            "total_cameras": 1,
            "abnormal_cameras": 1,
    ...
    w.finish_scan_run(
        run2,
        finished_at="2026-07-03T06:02:00Z",
        status="success",
        stats={
            "total_cameras": 1,
            "abnormal_cameras": 1,
...
def test_insert_events_new_topic_inserts(db_path):
    ...
    topics = [
        r[0] for r in conn.execute("SELECT DISTINCT event_topic FROM events").fetchall()
    ]
    conn.close()
    assert n == 2
...
def test_insert_events_resolved_allows_reinsert(db_path):
    ...
    conn = sqlite3.connect(db_path)
    conn.execute("UPDATE events SET resolved_at = '2026-07-03T05:30:00Z' WHERE id = 1")
    conn.commit()
    conn.close()
    ...
    w.finish_scan_run(
        run3,
        finished_at="2026-07-03T07:02:00Z",
        status="success",
        stats={
            "total_cameras": 1,
            "abnormal_cameras": 1,
