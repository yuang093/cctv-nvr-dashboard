...
def _temp_db_with_cams(cam_specs: list[str]):
    ...
    nvr_int = w.upsert_nvr({
        "id": "NVR-T", "name": "test nvr", "host": "10.0.0.1",
    ...
    for i, kind in enumerate(cam_specs, start=1):
        device_id = f"dev-{i}"
        if kind == "online":
            cam_data[device_id] = {"name": f"cam{i}",
            ...
            if kind == "signal_lost":
                event_data.append({
                    "eventId": f"e{i}", "deviceId": device_id,
            ...
            cam_data[device_id] = {"name": f"rtsp://ghost{i}",
                                   "connection_state": "DISCONNECTED"}
    ...
    w.finish_scan_run(
        rid, finished_at="2026-07-31T00:01:00Z", status="complete",
        stats={"total_cameras": len(cam_specs), "abnormal_cameras": len(event_data),
    ...
    if ghost_device_ids:
        ...
        for did in ghost_device_ids:
            conn.execute(
                "UPDATE cameras SET is_ghost = 1 WHERE device_id = ?", (did,))
        conn.commit()
        conn.close()
...
def test_health_distribution_empty():
    ...
    with _temp_db_with_cams([]) as db_path:
        ...
        d = get_camera_health_distribution(db_path)
        assert d == {
            "online": 0, "signal_lost": 0, "no_signal": 0,
