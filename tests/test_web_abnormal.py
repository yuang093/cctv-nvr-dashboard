...
def _seed_abnormal(db_path: str) -> None:
    ...
    nvr_a = w.upsert_nvr({
        "id": "NVR-A", "name": "A 大樓", "host": "1.1.1.1",
        "port": 8443, "username": "u", "password": "p",
    })
    nvr_b = w.upsert_nvr({
        "id": "NVR-B", "name": "B 大樓", "host": "2.2.2.2",
        "port": 8443, "username": "u", "password": "p",
    })
    ...
    w.finish_scan_run(
        run_id, finished_at="2026-07-03T10:15:00Z",
        status="success", stats={
            "total_cameras": 3, "abnormal_cameras": 2,
            "total_nvrs": 2, "ok_nvrs": 2,
        },
    )
    del w
...
def test_run_detail_no_abnormal_no_pdf_link(client, abn_app):
    ...
    run_id = w.begin_scan_run("2026-07-03T10:00:00Z")
    w.finish_scan_run(
        run_id, finished_at="2026-07-03T10:01:00Z",
        status="success", stats={"total_cameras": 0, "abnormal_cameras": 0,
                                  "total_nvrs": 0, "ok_nvrs": 0},
    )
    del w
...
def test_state_topic_in_run_detail(client, abn_app):
    ...
    w = SqliteWriter(db_path)
    nvr_a = w.upsert_nvr({
        "id": "NVR-S", "name": "S 大樓", "host": "1.1.1.1",
        "port": 8443, "username": "u", "password": "p",
    })
    run_id = w.begin_scan_run("2026-07-03T10:00:00Z")
    ...
    w.finish_scan_run(
        run_id, finished_at="2026-07-03T10:10:00Z",
        status="success", stats={"total_cameras": 1, "abnormal_cameras": 1,
...
def test_get_abnormal_cameras_grouped_filters_orphan_events(client, abn_app):
    ...
    # orphan event 必須被過濾掉
    assert "ghost-cam" not in device_ids, \
